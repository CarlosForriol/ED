...
void actualizacion(MapTree<string, int>& nuevo, MapTree<string, int>& viejo) {
	...
	for (auto& it : viejo) {
		if (!nuevo.contains(it.key))eliminados.push_back(it.key);
			...
			if (it.value != nuevo[it.key]) {
				modificados.push_back(it.key);
			}
			nuevo.erase(it.key);
		}
	}
	if (eliminados.empty() && nuevo.empty() && modificados.empty())cout << "Sin cambios" << endl;
	if (!nuevo.empty()) {
		cout << "+ ";
...
void tratar_caso() {
	...
	getline(cin, linea);
	MapTree<string, int> diccAntiguo;
	stringstream viejo(linea);
	while (viejo >> palabra) {
		viejo >> diccAntiguo[palabra];
	}

	getline(cin, linea);
	MapTree<string, int> diccNuevo;
	stringstream nuevo(linea);
	while (nuevo >> palabra) {
		nuevo >> diccNuevo[palabra];
	}
	actualizacion(diccNuevo, diccAntiguo);
	cout << "---"<<endl;
}
