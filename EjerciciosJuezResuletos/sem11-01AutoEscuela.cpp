...
public:
	...
	void aprobar(const string alumno) {
		if (!alumnos.count(alumno))throw std::domain_error("El alumno " + alumno + " no esta matriculado");
		string alumnoBorrar = buscarAlumno(alumno).profesor;
		profesores[alumnoBorrar].erase(alumno);
...
private:
	...
	InfoAlumno& buscarAlumno(const string& alumno) {
		...
		if(it == alumnos.end())throw std::domain_error("El alumno " + alumno + " no esta matriculado");
		return it->second;
	}
