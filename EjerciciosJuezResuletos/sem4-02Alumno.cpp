...
#endif
int alumno(int cant, int salvado) {
	ListLinkedDouble<int> colaDoble;
	for (int i = 0; i < cant; i++) {
		colaDoble.push_back(i + 1);//añadimos numeros
	}
	auto it = colaDoble.begin();

	while (colaDoble.size() > 1) {
		int i = 0;
		while (i < salvado) {
			int elegida = *it;
			it = colaDoble.erase(it);
			colaDoble.push_back(elegida);
			i++;
		}
