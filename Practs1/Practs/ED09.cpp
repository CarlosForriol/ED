...
void desplazar(int numDespacho, MapTree<int, int>& t, int c, int numP) {
		...
		while (numP != 0) {
			if (numP == -1) {//jefe
				...
				if (t[numDespacho]!= -1) {
					if (t[numDespacho] + numP <= c) {// si las q habia y quieres meter caben
						t[numDespacho] += numP;//sumas a los q estaban despacho los nuevos
						numP = 0;
					}
					else {
						numP = numP + t[numDespacho] - c;//las personas q no han cabido despacho
						t[numDespacho] = c;//lo llenas
					}
				}
