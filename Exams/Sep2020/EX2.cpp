...
pair<bool, int> es_diestro_aux(const BinTree<int>& arbol) {
	if (arbol.empty())return { true,0 };//Es vacio
	if (arbol.left().empty() && arbol.right().empty())return { true, arbol.root() };//Es una hoja

	auto [esDiestroIzq, sumIzq] = es_diestro_aux(arbol.left());
	auto [esDiestroDer, sumDer] = es_diestro_aux(arbol.right());

	return { esDiestroDer && esDiestroIzq && sumDer > sumIzq, sumDer + sumIzq  +arbol.root()};
