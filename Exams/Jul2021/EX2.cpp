...
pair<bool, int> bien_codificado_aux(const BinTree<int>& arbol) {
	...
	if (arbol.root() == 0) {
		return { codificadoIzq && codificadoDer && abs(cerosIzq -cerosDer) <=1 ,cerosIzq + cerosDer + 1 };
	}
	else {
		return { codificadoIzq && codificadoDer && abs(cerosIzq - cerosDer) <= 1,cerosIzq + cerosDer };
	}
