...
public:
	...
	void pedirConsulta(const string& paciente, const string& medico) {
		...
		if(pacientes[paciente].espera.count(medico))throw domain_error("Paciente ya esperando");
	...
	void atenderConsulta(const string& medico) {
		...
		pacientes[paciente].espera.erase(medico);
	}

	list<string> abandonarConsulta(const string& paciente) {
		if (!pacientes.count(paciente)) throw domain_error("Paciente inexistente");
		list<string> lista;
		InfoPaciente info = pacientes[paciente];
		for (auto it : info.espera) {
...
private:
	...
	typedef struct {
		map<string, list<string>::iterator>espera;
	}InfoPaciente;
