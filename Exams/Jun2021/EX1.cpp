...
public:
	...
	void unzip(const ListLinkedDouble& dest);
...
template<typename T>
void ListLinkedDouble<T>::dettach(Node* node) {
...
template<typename T>
void ListLinkedDouble<T>::attach(Node* node, Node* before) {
...
template<typename T>
void ListLinkedDouble<T>::unzip(const ListLinkedDouble& dest) {
	Node* curr = head->next;
	int i = 0;
	while (curr != head) {
		if (i % 2 == 1) {
